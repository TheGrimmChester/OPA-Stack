...
ORA (`:8091`) owns SCM/GitHub **protocol** and AI review. Connector **management** UI and public write BFF live on OAM (`:8097` / NAS `:18097`).

| Where | Enabled |
|-------|---------|
| **osa-api** | Security runs, findings, vulns/IAST, AppSec gate (`OSA_HTTP` / `:8093`) |
| **ora-api** | SCM protocol, review, optional gate wait on OSA; gitleaks workspace when used for review-local scans |
| **oam-api** | Connector directory + public CRUD BFF (peers to ORA); credentials and model bindings |
