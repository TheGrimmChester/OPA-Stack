...
|------|----------|--------------------|-------------|
| `Open-Client-JS` | Typed control-plane HTTP clients + user-auth header helpers | `*-Dashboard` | npm `@open-family/client` |
...
| `Open-Job-Go` | Sandboxed job lifecycle, hardened `docker run`, labels, env scrub | Product orchestrators | `github.com/TheGrimmChester/open-job-go` |
...
| `Open-UI-JS` | Dashboard shell kit (layout, session helpers, tokens) | `*-Dashboard` | npm `@open-family/ui` |
...
Modules use semver. Products pin versions. Modules remain private while the family is private.
