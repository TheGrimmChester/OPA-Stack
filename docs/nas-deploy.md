...
OPM/OSA GitHub targets: `opm-api` and `osa-api` use `PEER_OPA_URL` (org directory) and `PEER_ORA_URL` (SCM protocol / clone credentials). Connector and AI credential **storage** lives in **OAM** (`PEER_OAM_URL`); configure GitHub App protocol on **ora-api** (`OPA_GITHUB_APP_*`, `OPA_CONNECTOR_SECRET` — must match `OAM_SECRET_KEY`, and must be **distinct from** `JWT_SECRET`). **`opm-api:nas` runtime must include `git` and the docker CLI**. Model-backed jobs: with `PEER_OAM_URL` set (NAS default), configure providers and per-agent bindings in the **OAM** console — jobs resolve model + API key per phase and fail closed without a credential. Legacy `OPM_MODEL_API_KEY` (+ `OPM_MODEL_*`) in `.env` applies only when `PEER_OAM_URL` is unset (rollback). Task jobs clone via ORA credentials and `docker run` `opm-runner-task:nas` when spawn-ready (compose mounts `docker.sock` on `opm-api` and `opm-orchestrator`). **`OPM_JOB_TMP` must be a host bind with the same path inside and outside `opm-api`** (default `/mnt/Apps/config-docker/open-stack/opm-jobs`) so sibling runner containers can mount the prepared clone at `/repo`; a named volume at `/tmp/opm-jobs` leaves `/repo` empty. After image upgrades, redeploy `opa-hub`, `oam-api`, `ora-api`, `opm-api`, `opm-orchestrator`, `opm-dashboard`, `osa-api`, and `osa-dashboard` (`*:nas` tags only).
