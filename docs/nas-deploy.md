...
OPM/OSA GitHub targets: `opm-api` and `osa-api` use `PEER_OPA_URL` (org directory) and `PEER_ORA_URL` (connectors / clone credentials). Configure GitHub App or PAT on **ora-api** (`OPA_GITHUB_APP_*`, `OPA_CONNECTOR_SECRET`). **`opm-api:nas` runtime must include `git` and the docker CLI**. Optional model-backed jobs: set `OPM_MODEL_API_KEY` (+ `OPM_MODEL_BASE_URL` / `OPM_MODEL`) in the stack `.env` (see `compose.nas.env.example`); without the key, container spawn still works and builtin artifacts remain the fallback — task jobs clone via ORA credentials and `docker run` `opm-runner-task:nas` when spawn-ready (compose mounts `docker.sock` on `opm-api` and `opm-orchestrator`). **`OPM_JOB_TMP` must be a host bind with the same path inside and outside `opm-api`** (default `/mnt/Apps/config-docker/open-stack/opm-jobs`) so sibling runner containers can mount the prepared clone at `/repo`; a named volume at `/tmp/opm-jobs` leaves `/repo` empty. After image upgrades, redeploy `opa-hub`, `ora-api`, `opm-api`, `opm-orchestrator`, `opm-dashboard`, `osa-api`, and `osa-dashboard` (`*:nas` tags only).
