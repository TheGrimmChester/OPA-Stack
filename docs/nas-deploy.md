...
| `OPM_RUNNER_NETWORK` | `internal+proxy` | Per-job sealed net + `open-egress-proxy`. Break-glass: `OPM_RUNNER_NETWORK=bridge` |
...
Set **`OPA_CONNECTOR_SECRET`** in `.env` before `docker compose up` — compose fails closed if it is unset (`${OPA_CONNECTOR_SECRET:?…}`). Use a secret (≥32 bytes) **distinct from** both `JWT_SECRET` and `OPEN_SERVICE_JWT_SECRET`. It must match **`OAM_SECRET_KEY`** (or leave `OAM_SECRET_KEY` unset so OAM falls back to `OPA_CONNECTOR_SECRET`). Older NAS compose aliased connector crypto to `JWT_SECRET` — that is no longer the default. If ciphertext was encrypted with the JWT material, set `OPA_CONNECTOR_SECRET` to that **same historical value** until you rotate/re-encrypt, then recreate `ora-api` and `oam-api` together.

OPM/OSA GitHub targets: `opm-api` and `osa-api` use `PEER_OPA_URL` (org directory) and `PEER_ORA_URL` (SCM protocol / clone credentials). Connector and AI credential **storage** plus the **Connectors management UI** live in **OAM** (`PEER_OAM_URL`; dashboard `:18097` / `/connectors`). Configure GitHub App protocol on **ora-api** (`OPA_GITHUB_APP_*`, `OPA_CONNECTOR_SECRET` — must match `OAM_SECRET_KEY`, and must be **distinct from** `JWT_SECRET`). Set **`OAM_DASHBOARD_URL`** on `ora-api` (NAS default `http://192.168.100.101:18097`) so App install/claim browsers land on OAM, not OPA `:8088`. **`opm-api:nas` runtime must include `git` and the docker CLI**. Model-backed jobs: with `PEER_OAM_URL` set (NAS default), configure providers and per-agent bindings in the **OAM** console — jobs resolve model + API key per phase and fail closed without a credential. Legacy `OPM_MODEL_API_KEY` (+ `OPM_MODEL_*`) in `.env` applies only when `PEER_OAM_URL` is unset (rollback). Task jobs clone via ORA credentials and `docker run` `opm-runner-task:nas` when spawn-ready (compose mounts `docker.sock` on `opm-api` and `opm-orchestrator`). **`OPM_JOB_TMP` must be a host bind with the same path inside and outside `opm-api`** (default `/mnt/Apps/config-docker/open-stack/opm-jobs`) so sibling runner containers can mount the prepared clone at `/repo`; a named volume at `/tmp/opm-jobs` leaves `/repo` empty. After image upgrades, redeploy `opa-hub`, `oam-api`, `oam-dashboard`, `ora-api`, `opm-api`, `opm-orchestrator`, `opm-dashboard`, `osa-api`, and `osa-dashboard` (`*:nas` tags only).
...
Two checks can block a merge: **OSA AppSec Gate** (findings, evaluated by
...
With `OPA_AUTH_REQUIRED=1`, OSA/OPL ClickHouse list endpoints do **not** silently fall back to `default-org` / `default-project` when `X-Organization-ID` / `X-Project-ID` are omitted — expect **empty**, **400**, or JWT-pinned own scope (personal owner-only / org pin). Platform admins list users/orgs without impersonation; for tenant product data, impersonate or send a concrete org. Canonical curl matrix: [security-tenant-scopes.md](security-tenant-scopes.md) (also [interop.md — Tenant headers](interop.md#tenant-headers-required-when-auth-is-on)). Run `HOST=192.168.100.101 ./harness/security-tenant-matrix.sh` before merging tenancy changes.
